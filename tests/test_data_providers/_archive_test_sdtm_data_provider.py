...
# Test case 1
def test_get_data_sdtm_output(dp, data_folder):
    dp.set_mode("schema_PROPERTY")
    # checking content of columns
    df_extracted=prepare_get_data_sdtm(dp, data_folder, metadata_version = "v4")
...
#Test case 2 (with schema_CLASS mode (req v5 metadata) )
def test_get_data_sdtm_output_maps_to_class(dp, data_folder):
    dp.set_mode("schema_CLASS")
    # checking content of columns
    df_extracted=prepare_get_data_sdtm(dp, data_folder, metadata_version = "v5")
    result1= df_extracted[['STUDYID', 'USUBJID', 'AGE', 'SITEID', 'RFSTDTC', 'SEX']]
    dl = file_data_loader.FileDataLoader(domain_dict={'testdata.xls': 'DM'})
    df = dl.read_file(folder=data_folder, filename="testdata.xls")[0]
